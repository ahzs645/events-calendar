...
import { CalendarDays, MapPin, Building2, Clock, Users, DollarSign, List, ArrowLeft, ArrowRight, ChevronLeft, ChevronRight, Building, Calendar } from "lucide-react";
...
const CustomMonthView = ({ events, onDateClick, onEventClick }: { events: Event[]; onDateClick?: (date: Date) => void; onEventClick?: (event: Event) => void }) => {
...
// Week View Component
const WeekView = ({ events, onEventClick }: { events: Event[]; onEventClick?: (event: Event) => void }) => {
...
// Day View Component  
const DayView = ({ events, initialDate, onEventClick }: { events: Event[]; initialDate?: Date; onEventClick?: (event: Event) => void }) => {
...
export default function UNBCCalendar() {
  ...
  const generateCalendarLink = (event: Event, type: 'google' | 'outlook' | 'apple') => {
    ...
    const endDate = event.endDate || new Date(startDate.getTime() + 60 * 60 * 1000); // Default 1 hour if no end date
    ...
    switch (type) {
      ...
      case 'apple':
        // For Apple Calendar, we'll use a data URL with .ics content
        const icsContent = [
          'BEGIN:VCALENDAR',
          'VERSION:2.0',
          'BEGIN:VEVENT',
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="month" className="p-6 pt-4">
            <div className="hidden md:block">
              <CustomMonthView events={filteredEvents} onDateClick={handleDateClick} onEventClick={handleEventClick} />
            </div>
          ...
          <TabsContent value="day" className="p-6 pt-4">
            <DayView events={filteredEvents} initialDate={selectedDate} onEventClick={handleEventClick} />
          </TabsContent>
            ...
            <>
              ...
              <div className="space-y-4 my-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">
                        ...
                        {selectedEvent.endDate && ` - ${selectedEvent.endDate.toLocaleTimeString('en-US', {
                          hour: 'numeric',
                          minute: '2-digit',
                          hour12: true
                        })}`}
                  ...
                  {eventMetadata[selectedEvent.id] && (
                    <>
                      <div className="flex items-center gap-3 text-sm">
                        <MapPin className="h-4 w-4 text-gray-500" />
                        <span>{eventMetadata[selectedEvent.id].location}</span>
                      </div>
                      
                      <div className="flex items-center gap-3 text-sm">
                        <Building2 className="h-4 w-4 text-gray-500" />
                        <span>{eventMetadata[selectedEvent.id].organization}</span>
                      </div>
                      
                      <div className="flex items-center gap-3 text-sm">
                        <DollarSign className="h-4 w-4 text-gray-500" />
                        <span>{eventMetadata[selectedEvent.id].cost}</span>
                      </div>
                      
                      <div className="flex items-center gap-3">
                        <Badge className={
                          eventMetadata[selectedEvent.id].category === 'academic' ? 'bg-green-100 text-green-800' :
                          eventMetadata[selectedEvent.id].category === 'social' ? 'bg-orange-100 text-orange-800' :
                          eventMetadata[selectedEvent.id].category === 'cultural' ? 'bg-purple-100 text-purple-800' :
                          eventMetadata[selectedEvent.id].category === 'sports' ? 'bg-red-100 text-red-800' :
                          eventMetadata[selectedEvent.id].category === 'professional' ? 'bg-teal-100 text-teal-800' :
                          eventMetadata[selectedEvent.id].category === 'wellness' ? 'bg-blue-100 text-blue-800' :
                          eventMetadata[selectedEvent.id].category === 'volunteer' ? 'bg-yellow-100 text-yellow-800' :
                          'bg-pink-100 text-pink-800'
                        }>
                          {eventMetadata[selectedEvent.id].category.charAt(0).toUpperCase() + eventMetadata[selectedEvent.id].category.slice(1)}
                        </Badge>
                        {eventMetadata[selectedEvent.id].registrationRequired && (
              ...
              <DialogFooter className="flex-col sm:flex-col gap-2">
                ...
                <div className="flex gap-2 w-full">
                  <Button
                    variant="outline"
                    className="flex-1"
                    onClick={() => window.open(generateCalendarLink(selectedEvent, 'google'), '_blank')}
                  >
                  ...
                  <Button
                    variant="outline"
                    className="flex-1"
                    onClick={() => window.open(generateCalendarLink(selectedEvent, 'outlook'), '_blank')}
                  >
                    <Calendar className="h-4 w-4 mr-2" />
                    Outlook
                  </Button>
                  <Button
                    ...
                    className="flex-1"
                    onClick={() => {
                      const link = generateCalendarLink(selectedEvent, 'apple');
